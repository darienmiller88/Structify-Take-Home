...
functions, I was able to craft a solution to the prompt that scales with 

### Question
Given a list of chords in a circle, count all intersections 
...
the first radian measure, or `sx` of the chord, and the second value representing the second radian measure, or `ex`. `x` in this case represents which number chord these measures belong to, the 1st one, 2nd one, 3rd, etc. `sx` and `ex` values must start at `1`, and continue by `1` for as many chords you are inputting in to program.